use eframe::egui::{self, Color32, RichText, Ui, Layout, Align, Vec2};
...
impl RustySmartStitchApp {
    ...
    fn save_waifu2x_path(&self) {
        if let Some(config_dir) = dirs::config_dir() {
            ...
            let _ = std::fs::write(
                config_dir.join("waifu2x_path.txt"),
                &self.waifu2x_exe_path
            );
        }
    }
    ...
    fn show_waifu2x_enable_toggle(&mut self, ui: &mut Ui) {
        ui.horizontal(|ui| {
            ...
            let response = ui.add_enabled(
                can_enable,
                egui::Checkbox::new(&mut self.waifu2x_enabled, 
    ...
    fn show_conversion_mode(&mut self, ui: &mut Ui) {
        ui.horizontal(|ui| {
            ...
            ui.add_space(10.0);
            ui.vertical(|ui| {
                ui.radio_value(&mut self.waifu2x_mode, "noise_scale".to_string(), "Denoise & Magnify");
                ...
                ui.radio_value(&mut self.waifu2x_mode, "auto_scale".to_string(), "Magnify & Auto Denoise");
            });
        });
    }
            ...
            ui.add_enabled_ui(denoise_enabled, |ui| {
                ui.vertical(|ui| {
                    for level in ["0", "1", "2", "3"] {
                        ui.radio_value(&mut self.waifu2x_noise_level, level.to_string(), format!("Level {}", level));
                    }
                });
            });
        });
    ...
    fn show_scale_options(&mut self, ui: &mut Ui) {
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.radio_value(&mut self.waifu2x_scale_mode, "ratio".to_string(), "Set rate");
                ui.add_space(5.0);
                ui.add_enabled(
    ...
    fn show_both_dimensions(&mut self, ui: &mut Ui, input_width: f32) {
        ui.horizontal(|ui| {
            ui.radio_value(&mut self.waifu2x_scale_mode, "both".to_string(), "Set width & height");
            ui.add_space(5.0);
            if self.waifu2x_scale_mode == "both" {
                ui.add_sized(
                    Vec2::new(input_width, 20.0),
                    egui::TextEdit::singleline(&mut self.waifu2x_scale_width)
                        .hint_text("width")
                );
                ui.label("x");
                ui.add_sized(
                    Vec2::new(input_width, 20.0),
                    egui::TextEdit::singleline(&mut self.waifu2x_scale_height)
                        .hint_text("height")
                );
            }
        });
    ...
    fn show_model_settings(&mut self, ui: &mut Ui) {
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_label("")
                ...
                .show_ui(ui, |ui| {
                    ui.selectable_value(&mut self.waifu2x_model, "anime_style_art_rgb".to_string(), "anime_style_art_rgb");
                    ui.selectable_value(&mut self.waifu2x_model, "anime_style_art".to_string(), "anime_style_art");
                    ui.selectable_value(&mut self.waifu2x_model, "photo".to_string(), "photo");
                    ui.selectable_value(&mut self.waifu2x_model, "upconv_7_anime_style_art_rgb".to_string(), "upconv_7_anime_style_art_rgb");
                    ui.selectable_value(&mut self.waifu2x_model, "upconv_7_photo".to_string(), "upconv_7_photo");
                    ui.selectable_value(&mut self.waifu2x_model, "upresnet10".to_string(), "upresnet10");
            ...
            ui.add_space(20.0);
            ui.checkbox(&mut self.waifu2x_tta, RichText::new("Use TTA Mode").size(14.0))
    ...
    fn show_processing_settings(&mut self, ui: &mut Ui) {
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                ui.add_sized(
                    Vec2::new(70.0, 20.0),
                    egui::TextEdit::singleline(&mut self.waifu2x_gpu)
                        .hint_text("0")
                );
            });
        });
    ...
    fn show_split_size(&mut self, ui: &mut Ui) {
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ...
                ui.radio_value(&mut self.waifu2x_split_mode, "default".to_string(), "Default")
                ...
                if self.waifu2x_split_mode == "custom" {
                    ...
                    ui.add_sized(
                        ...
                        egui::TextEdit::singleline(&mut self.waifu2x_crop_w)
                            .hint_text("Width")
                    );
                    ui.label("x");
                    ui.add_sized(
                        Vec2::new(70.0, row_height),
                        egui::TextEdit::singleline(&mut self.waifu2x_crop_h)
                            .hint_text("Height")
                    );
                }
            });
    ...
    fn show_batch_size(&mut self, ui: &mut Ui) {
        ui.vertical(|ui| {
            ...
            ui.add_sized(
                Vec2::new(70.0, 20.0),
                egui::TextEdit::singleline(&mut self.waifu2x_batch_size)
                    .hint_text("1")
            );
        });
    }
    ...
    fn show_output_depth(&mut self, ui: &mut Ui) {
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.add_sized(
                    Vec2::new(50.0, 20.0),
                    egui::TextEdit::singleline(&mut self.waifu2x_output_depth)
                        .hint_text("8")
                );
                ui.label("bit");
            });
