...
impl RustySmartStitchApp {
                ...
                ui.group(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.add(
                            egui::TextEdit::singleline(&mut self.edges)
                                .desired_width(70.0)
                                .hint_text("1-20")
                        ).on_hover_text("Number of pixels to ignore at edges when detecting slice points");
                    });
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.add_enabled_ui(self.upscale_enabled, |ui| {
                            ui.horizontal(|ui| {
                                for factor in [1, 2, 3] {
                                    ui.radio_value(&mut self.upscale_factor, factor, format!("x{}", factor));
                                }
                            });
                        }).response.on_hover_text("Scale the image by a fixed multiplier");
                    });
                ...
                ui.group(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.add_space(10.0);
                            if ui.add_sized(Vec2::new(80.0, 24.0), egui::Button::new("💾 Save"))
                                .clicked() && !self.current_profile_name.is_empty() {
                            ...
                            egui::ComboBox::new("profile_select", "Select a profile")
                                ...
                                .show_ui(ui, |ui| {
                                    for (name, _) in &self.profiles {
                                        ui.selectable_value(&mut self.current_profile_name, name.clone(), name);
                                    }
                                });
                        });
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if !profile_name.is_empty() {
                                if ui.add_sized(Vec2::new(80.0, 24.0), egui::Button::new("📂 Load"))
                                    .clicked() {
                                    self.load_profile(&profile_name);
                                }
                                ui.add_space(8.0);
                                if ui.add_sized(Vec2::new(80.0, 24.0), egui::Button::new("🗑️ Delete"))
                                    .clicked() {
                                    if let Some(pos) = self.profiles.iter().position(|(n, _)| n == &profile_name) {
                                        self.profiles.remove(pos);
                                        let _ = self.save_profiles();
                                        self.current_profile_name.clear();
                                        self.profile_message = format!("Deleted profile: {}", profile_name);
                                    }
                                }
                                ui.add_space(8.0);
                                if ui.add_sized(Vec2::new(80.0, 24.0), egui::Button::new("📤 Export"))
                                    .clicked() {
                            ...
                            if ui.add_sized(Vec2::new(80.0, 24.0), egui::Button::new("📥 Import"))
                                .clicked() {
    ...
    pub fn has_active_advanced_settings(&self) -> bool {
        ...
        (self.resize_enabled && (!self.resize_width.is_empty() || !self.resize_height.is_empty()))
    }
}